...
# Description: Pre-installation test for (GVM 22.4.x) on Kali Linux 2023.3
...
# allow users of the _gvm group run openvas
...
sudo /usr/bin/greenbone-feed-sync
...
ExecStart=/usr/local/sbin/gsad --foreground --listen=192.168.88.231 --port=9392 --http-only
Restart=always
TimeoutStopSec=10
